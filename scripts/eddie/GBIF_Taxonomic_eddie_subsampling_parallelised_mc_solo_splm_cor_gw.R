...
subsampling.plants <- function(spec_n) {
  ...
  if (nrow(species_sample) %in% seq(1,nrow(plantlist_names),50000)){
    tf1 <- paste0("/exports/eddie/scratch/s2117440/data/taxonomic/checkpoints/checkpoint_tax_gbif",
...
plantlist_dist <- gbif_dist %>% 
  ...
  dplyr::select(plant_name_id, LEVEL3_COD, family)
...
res_file <- paste0("/exports/eddie/scratch/s2117440/data/taxonomic/Samples_iteration_splm_tax_gbif",

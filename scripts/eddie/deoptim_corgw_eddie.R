...
subsampling.plants.solo <- function(spec_n) {
  ...
  if (cor.gwr > max(maxima)) {
    ...
    write.table(c(cor.gwr,ids), paste0(
      "/exports/eddie/scratch/s2117440/data/optimised/pops/",
...
#test <-  subsampling.plants.solo(30000)
...
solutions_listed <- mclapply(seq(1, ceiling(nrow(plantlist_names) /10), 1), mc.cores = 60, deoptim_lapply)

solutions <- do.call(rbind, solutions_listed)
write.table(solutions, "/exports/eddie/scratch/s2117440/data/optimised/DEoptim_results_corgw.txt")

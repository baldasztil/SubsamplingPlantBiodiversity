...
subsampling.plants <- function(spec_n) {
  ...
  if (nrow(species_sample) %in% seq(10,nrow(plantlist_names),50000)){
    tf1 <- paste0("/exports/eddie/scratch/s2117440/data/phylogenetic/checkpoints/checkpoint_phylo_gbif",nrow(species_sample),".txt")
...
plantlist_dist_phylo <- gbif_dist %>% 
  left_join(plantlist_names, by = "plant_name_id") %>% 
  dplyr::select(plant_name_id, taxon_name, LEVEL3_COD) %>% 
...
richness_patterns <- as.data.frame(PD(dist.mat, prune_tree$phy), 
                                   col.names =c("richness")) %>% 
  mutate(LEVEL3_COD = rownames(.)) %>% 
  rename(richness = `PD(dist.mat, prune_tree$phy)`) %>% 
...
res_file <- paste0("/exports/eddie/scratch/s2117440/data/phylogenetic/Samples_iteration_splm_phylo_gbif",sample(1:10000000, 1, replace=TRUE),".txt.gz")

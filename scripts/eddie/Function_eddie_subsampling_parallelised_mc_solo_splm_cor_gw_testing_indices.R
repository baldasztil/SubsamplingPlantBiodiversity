...
subsampling.plants <- function(spec_n, q) {
  ...
  shannon_div_sample <- as.data.frame(hill_taxa(table_data_sample,q = q))
...
continent_names_vec <- unique(tdwg_3$LEVEL1_NAM)
...
shannon_div <- as.data.frame(hill_taxa(table_data,q = 100)) 
...
test <- subsampling.plants(349113, q= 100)
test
# compare models ---------------------------------------------------------------
...
# q1 
shannon_div <- as.data.frame(hill_taxa(table_data,q = 1)) 
shannon_div$C <- rownames(shannon_div)
names(shannon_div) <- c("richness", "LEVEL3_COD")
...
bw_global <-bw.gwr(richness ~ richness2, rich_overall_bru_mid_shp, 
                   approach = "AIC",
                   adaptive = T)
result_list <- lapply(seq(1,100000 ,100), 
                      subsampling.plants, q= 1)

resultsq1 <- rbindlist(result_list) %>% 
  mutate(q = 1)

# q2 
shannon_div <- as.data.frame(hill_taxa(table_data,q = 2)) 
shannon_div$C <- rownames(shannon_div)
names(shannon_div) <- c("richness", "LEVEL3_COD")
...
bw_global <-bw.gwr(richness ~ richness2, rich_overall_bru_mid_shp, 
                   ...
                   adaptive = T)
result_list <- lapply(seq(1,100000 ,100), 
                      subsampling.plants, q= 2)

resultsq2 <- rbindlist(result_list) %>% 
  mutate(q = 2)



# q10
shannon_div <- as.data.frame(hill_taxa(table_data,q = 10)) 
shannon_div$C <- rownames(shannon_div)
names(shannon_div) <- c("richness", "LEVEL3_COD")
...
bw_global <-bw.gwr(richness ~ richness2, rich_overall_bru_mid_shp, 
                   ...
                   adaptive = T)
result_list <- lapply(seq(1,100000 ,100), 
                      subsampling.plants, q= 10)

resultsq10 <- rbindlist(result_list) %>% 
  mutate(q = 10)
...
bw_global <-bw.gwr(richness ~ richness2, rich_overall_bru_mid_shp, 
                   ...
                   adaptive = T)
result_list <- lapply(seq(1,100000 ,100), 
                      subsampling.plants, q= 100)

resultsq100 <- rbindlist(result_list) %>% 
  mutate(q = 100)
...
results <- rbind(resultsq1, resultsq2, resultsq10, resultsq100)
...
ggplot(results_global, aes(x = log10(sp), y = cor.sp_gwr_mean, col = q)) + 
  geom_line() 

write.csv(results, "q_analysis_results.csv")



test2 <- dist_native_calc %>% 
...
species_sample <- sample_n(plantlist_names, 15000)
dist <- plantlist_dist %>% 
  filter(plant_name_id %in% species_sample$plant_name_id)
table_data_sample <- table(dist$LEVEL3_COD, dist$growth_form)


test_countries <- rich_overall_bru %>% 
  group_by(LEVEL1_NAM) %>% 
  slice_sample( n = 1)

countries <- rich_overall_bru


table_data_samp <- table_data_sample[rownames(table_data_sample)  %in% test_countries$LEVEL3_COD, ]
table_data_origin <- table_data[rownames(table_data) %in% rownames(table_data_samp), ]

table_data_origin
table_data_samp

round(prop.table(table_data_samp, 1), 3)
round(prop.table(table_data_origin, 1), 3)

rowSums(abs(round(prop.table(table_data_samp, 1), 3) - round(prop.table(table_data_origin,1), 3))) 
as.data.frame(diversity(table_data_origin, index = "shannon")) 
as.data.frame(diversity(table_data_samp, index = "shannon")) 
...
as.data.frame(diversity(table_data_origin, index = "shannon")) 
vegan::fisher.alpha(table_data_origin)
as.data.frame(diversity(table_data_samp, index = "shannon")) 


subsampling.plants(12000)

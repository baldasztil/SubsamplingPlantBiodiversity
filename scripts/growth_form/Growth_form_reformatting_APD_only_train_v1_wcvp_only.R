...
data_merger <- data %>%  
  ...
  mutate(wood1 = ifelse(is.na(wood1), "check", wood1),
        growth_gift = ifelse(is.na(growth_gift), "check", growth_gift))
...
checks_growth_gift <- data_ref %>% 
  filter(is.na(growth_gift) & !is.na(wood1))

checks_wood1 <- data_ref %>% 
  filter(is.na(wood1) & !is.na(growth_gift))
...
data_na_wood <- data_na %>% 
  filter(!plant_name_id %in% data_na_growth$plant_name_id) %>% 
  slice_sample(n = na_wood1) %>% 
  mutate(wood1 = NA)
  
data_na_both <- data_na %>% 
  filter(!plant_name_id %in% data_na_growth$plant_name_id & !plant_name_id %in% data_na_wood$plant_name_id) %>% 
...
data_na_full <- data_na %>% 
  filter(!plant_name_id %in% data_na_growth$plant_name_id & 
           !plant_name_id %in% data_na_wood$plant_name_id &
           !plant_name_id %in% data_na_both$plant_name_id) %>% 
  bind_rows(data_na_growth, data_na_wood, data_na_both) %>% 
  arrange(plant_name_id)
...
data_test <-  data_sim %>% 
  ...
  mutate(growth_gift = ifelse(is.na(growth_gift),"check", growth_gift), 
         wood1 = ifelse(is.na(wood1),"check", wood1))
...
data_test <-  data_sim %>% 
  ...
  mutate(growth_gift = ifelse(growth_gift == "check",NA, growth_gift), 
         wood1 = ifelse(wood1 == "check",NA, wood1))
...
data_test <-  data_sim %>% 
  dplyr::select(plant_name_id, state, genus, continent, family, growth_gift, wood1, lifeform_description) %>% 
  mutate(growth_gift = ifelse(growth_gift == "check",NA, growth_gift), 
         wood1 = ifelse(wood1 == "check",NA, wood1))
...
mice_rf_imputed_v3 <- miceRanger(data_test_ref, 
                                 vars = c("state", "growth_gift", "wood1"), 
                                 returnModels = T, 
                                 m = 3, 
                                 maxiter = 5, 
                                 valueSelector = "value", 

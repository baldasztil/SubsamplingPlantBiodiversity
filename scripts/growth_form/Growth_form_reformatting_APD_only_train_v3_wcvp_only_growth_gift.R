...
summary.stats <- function(x, v1, apd_plant_growth_form_origin) {
...
lifeform <- read.csv("data/traits_all/lifeform_mapping_original.csv") %>% 
  ...
  mutate(apd_plant_growth_form = gsub(" ", "", lifeform$apd_plant_growth_form))
...
wcvp_accepted_merger <- plants_full %>%   
  ...
  mutate(old_lifeform = ifelse(old_lifeform == "", NA, old_lifeform))
...
write.csv(growth_summary, "lifeforms_032023.csv")
...
xtabs(~ apd_plant_growth_form , data = wcvp_accepted_merger)
...
data <- wcvp_accepted_merger %>% 
  ...
  mutate(growth_gift = ifelse(growth_gift =="subshrub", "shrub", growth_gift))


xtabs(~ apd_plant_growth_form + growth_gift , data = data)
xtabs(~ growth_gift , data = data)
xtabs(~ apd_plant_growth_form , data = data)
...
data_test <-  data_sim %>% 
  dplyr::select(plant_name_id, apd_plant_growth_form, genus, continent, family, growth_gift)
data_test_ref <- as.data.frame(unclass(data_test),stringsAsFactors=TRUE)
str(data_test_ref)
...
data_test <-  data_sim %>% 
  dplyr::select(plant_name_id, apd_plant_growth_form, genus, continent, family, lifeform_description)
data_test_ref <- as.data.frame(unclass(data_test),stringsAsFactors=TRUE)
str(data_test_ref)
check_na_in(data_test)
...
data_test <-  data_sim %>% 
  dplyr::select(plant_name_id, growth_gift, genus, continent, family)
data_test_ref <- as.data.frame(unclass(data_test),stringsAsFactors=TRUE)
str(data_test_ref)
check_na_in(data_test)
...
v2 <- as.data.frame(getVarImps(mice_rf_imputed_v2, 1:3, var = "growth_gift"))
perf_v2 <- summary.stats(3, v2, apd_plant_growth_form_origin = growth_gift)
perf_v2$v <- "v2"
...
data_state_small <- data_state %>% 
  ungroup() %>%  
  dplyr::select(plant_name_id, apd_plant_growth_form, count) %>% 
  filter(!duplicated(.))
...
data_tables <- data_multiapd_plant_growth_form_exam %>%
  ungroup() %>% 
  dplyr::select(plant_name_id, apd_plant_growth_form) %>% 
  filter(!duplicated(.))

tab <- xtabs(~apd_plant_growth_form , data = data_tables)

data_impt_exam <- data_multiapd_plant_growth_form_exam %>% 
  ungroup() %>% 
...
data_multiapd_plant_growth_form_rel <- data_impt_exam %>% 
  group_by(plant_name_id) %>% 
  slice_max(rel, n = 1) %>% 
  filter(rel > 0.5)
...
# ectract all 
data_multiapd_plant_growth_form_exam_split <- data_impt_exam %>% 
  filter(rel== 0.5)

huh <- data_multiapd_plant_growth_form_exam_split %>% 
  filter(duplicated(plant_name_id))

huh2 <- data_multiapd_plant_growth_form_exam_split %>% 
  filter(!duplicated(plant_name_id))

huh3 <- huh %>% 
  filter(duplicated(plant_name_id))

length(unique(data_multiapd_plant_growth_form_exam_split$plant_name_id))
...
data_apd_plant_growth_form_small_real <- data_origin_stat %>% 
  dplyr::select(plant_name_id, apd_plant_growth_form) %>% 
  filter(!duplicated(plant_name_id))

cramerxx <- data_imputed %>% 
  ungroup() %>%  
  dplyr::select(plant_name_id, continent, imp_apd_plant_growth_form = apd_plant_growth_form) %>% 
  filter(!duplicated(.)) %>% 
  right_join(data_apd_plant_growth_form_small_real, by = "plant_name_id")


tab <- xtabs(~apd_plant_growth_form + imp_apd_plant_growth_form , data = cramerxx)
...
actuals <- as.factor(cramerxx$apd_plant_growth_form)
preds <- as.factor(cramerxx$imp_apd_plant_growth_form)

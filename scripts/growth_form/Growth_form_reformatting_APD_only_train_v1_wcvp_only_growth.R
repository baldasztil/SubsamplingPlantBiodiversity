...
data <- wcvp_accepted_merger %>% 
  ...
  mutate(growth_gift = ifelse(is.na(growth_gift), wood1, growth_gift))
...
checks_growth_gift <- data_ref %>% 
  filter(is.na(growth_gift))

checks_life <- data_ref %>% 
  filter(is.na(lifeform_description) & is.na(humphreys_lifeform))
...
na_growth_gift <- round(nrow(data_na_toinsert) * (nrow(checks_growth_gift) / nrow(data_ref)))
...
check_na_in(data_na_life)

data_na_gift_wood <- data_na %>% 
  filter(!plant_name_id %in% data_na_life$plant_name_id) %>% 
  slice_sample(n = na_growth_gift) %>% 
  mutate(growth_gift = NA, 
         wood1 = NA)

data_na_full <- data_na %>% 
  filter(!plant_name_id %in% data_na_life$plant_name_id & 
           !plant_name_id %in% data_na_gift_wood$plant_name_id) %>% 
  bind_rows(data_na_life, data_na_gift_wood) %>% 
  arrange(plant_name_id)
...
data_test <-  data_sim %>% 
  dplyr::select(plant_name_id, apd_plant_growth_form, genus, continent, family, growth_gift, wood1, lifeform_description, humphreys_lifeform) %>% 
  mutate(growth_gift = ifelse(is.na(growth_gift),"", growth_gift), 
         wood1 = ifelse(is.na(wood1),"", wood1),
         lifeform_description = ifelse(is.na(lifeform_description),"", lifeform_description), 
         humphreys_lifeform = ifelse(is.na(humphreys_lifeform),"", humphreys_lifeform))
...
data_test <-  data_sim %>% 
  dplyr::select(plant_name_id, apd_plant_growth_form, genus, continent, family, growth_gift, wood1, lifeform_description) %>% 
  mutate(growth_gift = ifelse(is.na(growth_gift),"", growth_gift), 
         wood1 = ifelse(is.na(wood1),"", wood1),
         lifeform_description = ifelse(is.na(lifeform_description),"", lifeform_description))
...
data_test <-  data_sim %>% 
  dplyr::select(plant_name_id, apd_plant_growth_form, genus, continent, family, growth_gift, wood1, humphreys_lifeform) %>% 
  mutate(growth_gift = ifelse(is.na(growth_gift),"", growth_gift), 
         wood1 = ifelse(is.na(wood1),"", wood1),
         humphreys_lifeform = ifelse(is.na(humphreys_lifeform),"", humphreys_lifeform))
...
v1_imp <- v1 %>%  pivot_longer(cols = starts_with("Dataset"), values_to = "apd_plant_growth_form_v1")

xtabs(~ Dataset_1 + apd_plant_growth_form_origin, data = v1)
xtabs(~ Dataset_2 + apd_plant_growth_form_origin, data = v1)
xtabs(~ Dataset_3 + apd_plant_growth_form_origin, data = v1)


v2 <- as.data.frame(getVarImps(mice_rf_imputed_v2, 1:3, var = "apd_plant_growth_form"))
v2_imp <- v2 %>%  pivot_longer(cols = starts_with("Dataset"), values_to = "apd_plant_growth_form_v2", names_to = "name_v2")
v2$plant_name_id <- data_test_na$plant_name_id
v2$apd_plant_growth_form_origin <- data_test_na$apd_plant_growth_form

xtabs(~ Dataset_1 + apd_plant_growth_form_origin, data = v2)
xtabs(~ Dataset_2 + apd_plant_growth_form_origin, data = v2)
xtabs(~ Dataset_3 + apd_plant_growth_form_origin, data = v2)


v3 <- as.data.frame(getVarImps(mice_rf_imputed_v3, 1:3, var = "apd_plant_growth_form"))
v3_imp <- v3 %>%  pivot_longer(cols = starts_with("Dataset"), values_to = "apd_plant_growth_form_v3", names_to = "name_v3")
...
xtabs(~ Dataset_2 + apd_plant_growth_form_origin, data = v3)
xtabs(~ Dataset_3 + apd_plant_growth_form_origin, data = v3)


v4 <- as.data.frame(getVarImps(mice_rf_imputed_v4, 1:3, var = "apd_plant_growth_form"))
v4_imp <- v4 %>%  pivot_longer(cols = starts_with("Dataset"), values_to = "apd_plant_growth_form_v4", names_to = "name_v4")
v4$plant_name_id <- data_test_na$plant_name_id
v4$apd_plant_growth_form_origin <- data_test_na$apd_plant_growth_form

xtabs(~ Dataset_1 + apd_plant_growth_form_origin, data = v4)
xtabs(~ Dataset_2 + apd_plant_growth_form_origin, data = v4)
xtabs(~ Dataset_3 + apd_plant_growth_form_origin, data = v4)

...
growth_form <- fread("data/traits_all/growth_rep_wcvp.txt", header = T, fill = T) %>% 
...
wcvp_accepted_merger <- wcvp_accepted %>%   
  ...
  left_join(growth_form, by = "plant_name_id") %>% 
...
quality <- checks %>% 
  group_by(lifeform_description, growth_form, humphreys_lifeform, wood1,state, apd_plant_growth_form) %>% 
  reframe(count = n()) %>% 
  mutate(quality_check = case_when(
    growth_form == "herb" & wood1 == "herbaceous" ~ "good",
    growth_form %in% c("shrub", "subshrub", "tree") & wood1 == "woody" ~ "good", 
    growth_form == "climber" ~ "good")) %>% 
  mutate(quality_check = ifelse(is.na(quality_check), "check", quality_check))


checks_checked <- checks %>% 
  left_join(quality, by = c("lifeform_description", "growth_form", "humphreys_lifeform", "wood1","state", "apd_plant_growth_form"))


check2 <- checks_checked %>% 
  group_by(quality_check, growth_form) %>% 
  reframe(n())

quality2 <- wcvp_accepted_merger %>% 
  group_by(lifeform_description, growth_form, humphreys_lifeform, wood1, state, apd_plant_growth_form) %>% 
  reframe(count = n())


quality <- wcvp_accepted_merger %>% 
  mutate(quality_check = case_when(
    growth_form == "herb" & wood1 == "herbaceous" ~ "good",
    growth_form %in% c("shrub", "subshrub", "tree") & wood1 == "woody" ~ "good", 
    growth_form == "climber" ~ "good")) %>% 
...
data <- quality %>% 
  dplyr::select(lifeform_description, growth_form, humphreys_lifeform, 
...
data_merger <- data %>%  
  ...
  rbind(data_ref) %>% 
  mutate(growth_form = ifelse(is.na(growth_form), "shrub", growth_form), 
...
data_test <-  data_merger %>% 
  ...
  dplyr::select(plant_name_id, growth_form, state, wood1, family, lifeform_description, region) %>% #humphreys_lifeform, 
...
data_test_ref <-  data_test_no_na %>% 
  #filter(family == "Fabaceae") %>% 
  dplyr::select(ID, growth_form, apd_plant_growth_form, state, region, wood1, humphreys_lifeform, family, genus, lifeform_description)
...
data_compare <- data_test_filled %>% 
  ...
  dplyr::select(state, imp_state, ID, growth_form, apd_plant_growth_form, region, wood1, humphreys_lifeform, family, genus)

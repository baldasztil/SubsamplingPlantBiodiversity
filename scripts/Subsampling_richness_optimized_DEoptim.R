...
clusterExport(cl=cl,  list("plantlist_names","plantlist_dist","rich_overall_bru"),
             envir=environment())


foreach(i = 1389:1390, .packages = c("tidyverse", "DEoptim")) %dopar% {
  de_result <- DEoptim(objective, lower = i, upper = i,
                       DEoptim.control(VTR = -Inf, strategy = 2,
                                       bs = FALSE, itermax = 100, CR = 0.5, F = 0.8, 
                                       storepopfreq = 1, trace = FALSE,
                                       cluster = cl))
  #cbind(de_result$optim$bestmem, de_result$optim$bestval*-1)
}
...
foreach(i = 1389:1390, .packages = c("tidyverse", "DEoptim")) %dopar% {
  ...
  de_result <- DEoptim(objective, lower = i, upper = i,
                       DEoptim.control(VTR = -Inf, strategy = 2,
                                       ...
                                       storepopfreq = 1, trace = 2, parallelType='foreach'))
...
mbm <- microbenchmark(
  "parallel_foreach" = { foreach(i = 1390:1390, .packages = c("tidyverse", "DEoptim"), .combine = "rbind") %dopar% {
    de_result <- DEoptim(objective, lower = i, upper = i,
                         DEoptim.control(VTR = -Inf, strategy = 2,
                                         bs = FALSE, itermax = 100, CR = 0.5, F = 0.8, 
                                         storepopfreq = 1, trace = 2,
                                         cluster = cl))
...
# Return negative correlation (since DEoptim minimizes)
  if(cor.sp <= 0.95)
    return(cor.sp + nrow(species)+100000)
  if(cor.sp >= 0.95) 
    return(0+nrow(species)*0.1)
  #return(ifelse(cor.sp >= 0.95, cor.sp, 0 + nrow(species)))
  #return(ifelse(cor.sp <= 0.95, cor.sp, cor.sp*-1))
}


de_result <- DEoptim(objective, lower = 1, upper = 1390,
                     DEoptim.control(VTR = 0, strategy = 2,
                                     bs = FALSE, itermax = 10000, CR = 0.5, F = 0.8, 
                                     storepopfreq = 1, trace = 50, parallelType='auto', 
                                     cluster = cl))
...
de_result <- DEoptim(objective, lower = 860, upper = 860,
                     DEoptim.control(VTR = -Inf, strategy = 2,
                                     bs = FALSE, itermax = 100, CR = 0.5, F = 0.8, 
                                     storepopfreq = 1, trace = TRUE, parallelType='auto', 
                                     cluster = cl))

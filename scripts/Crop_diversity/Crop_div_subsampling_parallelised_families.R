...
parallel.subsampling <- function (iteration, plant_families) {
  ...
  for (i in 1:iteration)  {
    samples[[i]] <- parLapply(clust, plant_families, subsampling.plants)
  }
...
######
clust <- makeCluster(30, outfile= "log_families.txt")
clusterExport(clust, c("dist_native","plantlist_dist", "plantlist","rich_overall_bru"))
...
write.txt(data_out, "families_rel_stats.txt")

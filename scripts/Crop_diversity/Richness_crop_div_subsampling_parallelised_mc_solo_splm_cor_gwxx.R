 ...
 model_comp <- rbindlist(lapply(continent_names_vec, splm.continents, 
  ...
  model_comp_out <- model_comp %>% 
    ...
    mutate_if(is.numeric,
              round,
              digits = 4) %>% 
    dplyr::select(AICc, logLik, continent, sp, mae, mpred, sumres2, mres, sdres, minres, maxres,
                  range, pseudo.r.squared, cor.sp_gwr_mean, cor.sp_gwr_sd, model_id)
  # cumulative pattern
    
  #cumulative pattern
  if (nrow(species_sample) %in% seq(1,nrow(plantlist_names),50000)){
    tf1 <- paste0("data/checkpoint_rich_",nrow(species_sample),".gz.parquet")
    
    write_parquet(model_comp_out, sink = tf1, compression = "gzip", compression_level = 6) 
  }
...
result_list <- lapply(seq(1,nrow(plantlist_names),10000), #
                             subsampling.plants)
results2 <- rep(result_list, 10000)
results <- rbindlist(results2)

data.frame(replicate(10,sample(0:1,1000,rep=TRUE)))


file <- paste0("data/Samples_iteration_splm_rich",
               sample(1:10000000, 1, replace=TRUE),".parquet")

write_parquet(results, file, compression = "uncompressed") 

gbif_parq <- read_parquet("data/Samples_iteration_splm_rich1034079.parquet", as_data_frame = T) 

gbif_parq2 <- gbif_parq %>% 
  mutate_if(is.numeric,
            round,
            digits = 4) 

write_parquet(gbif_parq, file, compression = "gzip", compression_level = 9) 

gbif_parq <- read_parquet("data/Samples_iteration_splm_rich5971301.parquet", as_data_frame = T) 

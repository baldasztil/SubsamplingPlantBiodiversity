...
srli <- read.csv("data/red/srli_full.csv")
...
Sys.time()
red <- rbindlist(results)
write.table(red, paste0("data/red/red_list/Samples_iteration_gwr_future_redlist",sample(1:10000000, 1, replace=TRUE),".txt")) 
...
#names
plants_full <- srli %>% 
  filter(plant_name_id %in% srli$plant_name_id) 
...
results <- future_lapply(seq(1,nrow(plantlist_names),1), subsampling.plants, future.seed = NULL, future.stdout = F)
Sys.time()
srli <- rbindlist(results)
write.table(srli, paste0("data/red/red_list/Samples_iteration_gwr_future_srli",sample(1:10000000, 1, replace=TRUE),".txt")) 

...
parallel.subsampling <- function (iteration, sample_n) {
  for (i in 84:iteration)  {
    samples <- list()
    print(paste0("This is iteration ", i)) 
    samples[[1]] <- mclapply(seq(0,nrow(plantlist_names),sample_n), mc.cores = 50, subsampling.plants)
    xx <- do.call(bind_rows, samples[[1]])
    write.table(xx, paste0("data/taxonomic_rep/Samples_iteration_",i,".txt"))
  }

...
Sys.time()
end <- rbindlist(results)
write.table(end, paste0("data/end_nonend/Samples_iteration_gwr_future_end",sample(1:10000000, 1, replace=TRUE),".txt")) 
...
results <- future_lapply(seq(1,nrow(plantlist_names),1), subsampling.plants, future.seed = NULL, future.stdout = F)
Sys.time()
nonend <- rbindlist(results)
write.table(nonend, paste0("data/end_nonend/Samples_iteration_gwr_future_nonend",sample(1:10000000, 1, replace=TRUE),".txt")) 

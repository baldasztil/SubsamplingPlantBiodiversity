...
subsampling.plants <- function(spec_n) {
  ...
  library(rstanarm)
  library(ggeffects)
  m <- stan_glm(richness ~ richness_sample, data=rich_rel2, iter = 1000, refresh = 0)
  plot(m, "trace")
  summary(m, digits = 5)
  plot(m, "hist")
  hist(m$residuals)
  x <- mean(rich_rel2$richness_sample)
  y_rep <- posterior_predict(m, newdata = data.frame(richness_sample = 0.06521739))
  y_rep <- posterior_predict(m, newdata = data.frame(richness_sample))
  a <- m$model$richness
  b <- m$fitted.values
  xx <- as.data.frame(cbind(a,b))
  xxx<- m$data$LEVEL3_COD
  xx$country <- xxx
  ...
  mod <- spmodel::splm(richness ~ richness_sample, data=rich_rel,
                       spcov_type =  c("exponential"))
  
  y <- mod$obdata
  xx <- predict(mod, newdata = y)
  y$predicted <- xx
  mean_abs_error <- sum(abs(mod$obdata$richness - xx))/length(mod$obdata$richness)
  mean_abs_error
  mean_root2_error <- sqrt(mean((y$richness - y$predicted)^2))
  mean_root2_error 
...
start <- Sys.time()
aa <- subsampling.plants(4000)
Sys.time() - start

result_list <- lapply(seq(1,nrow(plantlist_names),1000), 
                             subsampling.plants)

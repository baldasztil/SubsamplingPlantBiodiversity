...
# This is a script to calculate the correlation between different families and 
#the global biodiversity patterns based on the WCVP. It is using the
# taxonomic and geographic information available through the data 
# to relate species richness in the a subsample of a given family 
# to global species richness patterns.
...
# this is a function to create an accumulative subsample for a given 
# plant family 
subsampling.plants <- function(plant_family) {
  
  
  # this  extracts all species of a given family
  plantlist_names <- plantlist %>% 
    filter(family %in% plant_family)
  list_rich_rel_cumulative <- list()
  
  # this creates an accumulative subsample
  ...
  # this creates richness patterns in the sample 
  dist <- dist_native %>% 
  ...
  # creating a dataset with the subsample and overall patterns of richness
  ...
  #extracting the cumulative patterns
  return(as.data.frame(do.call(bind_rows, list_rich_rel_cumulative)))
}

# this is a for loop around the subsampling.plants function

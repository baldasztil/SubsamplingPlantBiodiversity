...
subsampling.plants <- function(spec_n) {
  ...
  model_error <- suppressWarnings(cube(rich_rel_preds, j = list(sum(abs(richness - preds))/length(richness),
                      ...
                      abs(max(residuals)) - abs(min(residuals))), 
                      by = c("LEVEL1_NAM")))
 ...
 model_comp <- rbindlist(lapply(continent_names_vec, splm.continents, 
  ...
  if (nrow(species_sample) %in% seq(1,nrow(plantlist_names),50000)){
    write.table(model_comp_out, paste0("data/richness/checkpoints/Check@_sampling",
                                   nrow(species_sample),"sp_wcvp.rds")) 
  }
...
saveRDS(results, file = paste0("data/richness/Samples_iteration_splm_corgw_wcvp",sample(1:10000000, 1, replace=TRUE),".rds")) 

...
splm.continents <-  function(x, rich_rel)  {
  ...
  if (class(output_glance)[1] =="try-error" & continent_data$sp[1] > 50000)  {
    ...
    if (class(output_glance_big) =="list")  {
...
subsampling.plants <- function(spec_n) {
  ...
  rich_rel <- dist %>% 
    group_by(LEVEL3_COD, growth_form) %>% 
    reframe(richness_growth = n_distinct(plant_name_id)) %>% 
    group_by(LEVEL3_COD) %>% 
    reframe(richness_sample = sd(richness_growth),
      cv_sample = sd(richness_growth) / mean(richness_growth) * 100) %>% 
...
growth <- read.csv("data/traits_all/growths_rf.csv") %>% 
...
richness_patterns <- dist_native_calc %>% 
  group_by(area_code_l3, growth_form) %>% 
  reframe(richness_growth = n_distinct(plant_name_id)) %>% 
  
  group_by(area_code_l3) %>% 
  reframe(richness = sd(richness_growth),
          cv = sd(richness_growth) / mean(richness_growth) * 100, 
          richness2 = sum(richness_growth)) %>% 
  rename(LEVEL3_COD = area_code_l3) %>% 
  left_join(continent_names, by = "LEVEL3_COD")

table_data <- table(dist_native_calc$area_code_l3, dist_native_calc$growth_form)
table_data2 <- table(dist$LEVEL3_COD, dist$growth_form)
library(vegan)
shannon_diversity <- as.data.frame(diversity(table_data, index = "shannon"))
shannon_diversity$C <- rownames(shannon_diversity)

shannon_diversity2 <- as.data.frame(diversity(table_data2, index = "shannon"))
shannon_diversity2$C <- rownames(shannon_diversity2)
shannon_div <- shannon_diversity %>% 
  left_join(shannon_diversity2, by = "C") 
names(shannon_div) <- c("observed", "country", "sample")
shannon_div <-shannon_div %>% 
  mutate( 
         log_observed = log(observed), 
         log_sample = log(sample)) %>% 
  filter(!is.infinite(log_sample))

hist(shannon_div$observed)
hist(shannon_div$sample)
hist(shannon_div$log_observed)
hist(shannon_div$log_sample)

xx <- lm(shannon_div$log_observed ~ shannon_div$log_sample)
xx <- lm(shannon_div$observed ~ shannon_div$sample)

summary(xx)
hist(xx$residuals)

plot(shannon_div$observed ~ shannon_div$sample)

plot(xx)
library(fitdistrplus)
library(gamlss)
a <- fitdistrplus::fitdist(xx$residuals, dist = "norm")
plot(a)
fitdistrplus::descdist(xx$residuals)
...
write.table(results, paste0("data/fullsamples/Samples_iteration_splm_",sample(1:10000000, 1, replace=TRUE),".txt")) 

...
biome_data <- read.table("data/biome_wcvp_data.txt", header = T)
...
dist_native <- dist_raw %>% 
  ...
  left_join(biome_data, by = "plant_name_id") 
...
for (i in 1:100)  {
  ...
  write.table(xx, paste0("data/biome/Samples_iteration_biome_",i,".txt"))
}

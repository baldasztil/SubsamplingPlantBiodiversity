...
match_phylo_comm <- function (phy, comm, delete_empty_rows = TRUE) {
  if (!(is.data.frame(comm) | is.matrix(comm) | inherits(comm, 
                                                         "Matrix"))) {
...
subsampling.plants <- function(spec_n) {
  ...
  prune_tree <- phyloregion::match_phylo_comm(output_tree,subset_matrix)
    # doesnt work right now with old tree
  rich_rel <- as.data.frame(PD(subset_matrix, prune_tree$phy), col.names =c("richness")) %>% 
  ...
  if (nrow(species_sample) %in% seq(1,nrow(plantlist_names),50000)){
    write.table(model_comp, paste0("data/phylorich/checkpoints/Check@_sampling",
...
PD(dist.mat, output_tree)
psr(dist.mat, output_tree)
...
write.table(results, paste0("data/phylorich/Samples_iteration_splm_",sample(1:10000000, 1, replace=TRUE),".txt")) 

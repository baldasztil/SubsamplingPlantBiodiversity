...
library(doParallel)
# Defining functions -----------------------------------------------------------
subsampling.plants.samples <- function(spec_n) {
  ...
  dist <- plantlist_dist %>% 
    ...
    left_join(species, by = "plant_name_id")
  #if (length(cumulative_namelist) %in% seq(1,nrow(plantlist_names),10000)) {
   # print(paste0("There are ", length(cumulative_namelist) ,
...
parallel.subsampling <- function (iteration,sample_n) {
  ...
  for (i in 1:iteration)  {
    ...
    samples[[i]] <- parLapply(clust,seq(0,nrow(plantlist_names),sample_n),
                             subsampling.plants.samples)
  }
  return(samples)
}
...
clust <- makeCluster(6)
clusterExport(clust, c("plantlist_dist","plantlist_names"))
clusterEvalQ(clust, library(tidyverse))
doParallel::registerDoParallel(clust)

samples_rel <- parallel.subsampling(1, 10000)

data_out <- as.data.frame(do.call(samples_rel, xx))
write.table(data_out, "full_samples_rel.txt")
######


data_out <- as.data.frame(do.call(bind_rows, samples_rel))
write.table(data_out, "full_samples_relxx.txt.gz")

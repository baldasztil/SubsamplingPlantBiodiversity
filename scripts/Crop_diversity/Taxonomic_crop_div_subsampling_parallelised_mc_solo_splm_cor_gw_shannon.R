...
subsampling.plants <- function(spec_n) {
  ...
  if (nrow(species_sample) %in% seq(1,nrow(plantlist_names),50000)){
    tf1 <- paste0("data/taxonomic/checkpoints/Samples_iteration_splm_tax_shannon",nrow(species_sample),".parquet")
...
table_data <- table(dist_native_calc$area_code_l3, dist_native_calc$family)


shannon_div <- as.data.frame(diversity(table_data, index = "shannon", equalize.groups = F )) 
...
plantlist_names <- plants_full %>%  
  dplyr::select(plant_name_id, taxon_rank, family, taxon_name, family)
...
test <- subsampling.plants(120000)
...
results <- global
file <- paste0("data/taxonomic/Samples_iteration_splm_tax_shannon",

...
plantlist_dist3 <- plantlist_dist2 %>% 
  left_join(plantlist_dist, by = c("area_code_l3", "plant_name_id"))

dist_native <- plantlist_dist3
...
Sys.time()
samples <- lapply(seq(1,nrow(plantlist_names),1000), subsampling.plants) 
Sys.time()

...
subsampling.plants <- function(spec_n) {
  ...
  # overall
  GWRbandwidth  <- gwr.sel(richness ~ richness_sample, data=rich_rel,
          coords=cbind(rich_rel$lon, rich_rel$lat), adapt = T, method = "aic")
  
  GWRbandwidth2  <- gwr.sel(richness ~ richness_sample, data=rich_rel2,
                           coords=cbind(rich_rel$lon, rich_rel$lat), adapt = T, method = "aic")

  
  gwr.model <- gwr(richness ~ richness_sample, data=rich_rel,
                  coords=cbind(rich_rel$lon, rich_rel$lat), adapt=GWRbandwidth , hatmatrix=TRUE, se.fit=TRUE) 
  
  gwr.model <- gwr(richness ~ richness_sample, data=rich_rel2, adapt=GWRbandwidth2 , hatmatrix=TRUE, se.fit=TRUE)
  ...
  rich_rel_shp_sp <-  as(rich_rel_shp, "Spatial")
   ...
   bw <- bw.gwr(richness ~ richness_sample, data=rich_rel_shp_sp,
               approach = "AIC",
               adaptive = T) 
   ...
   # gwr.model <- ggwr(richness ~ richness_sample, data=rich_rel,

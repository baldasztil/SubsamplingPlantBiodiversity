...
samples_cumulative_rel <- sample_data %>% 
  ...
  collect() %>% 
  setDT() %>% 
  mutate(index =   gsub("\\d+\\.parquet", "", index))
  

threshold_corgw <- samples_cumulative_rel %>% 
...
curves_global <-  ggplot(data = samples_global, aes(x=log10(sp), y=mean_corgw)) +
  geom_line(data = samples_global, aes(color = index), 
            alpha = 0.8, lwd = 0.25, show.legend = T) +
...
ggsave("curves_global.png", curves_global, width = 10, height = 5, dpi = 300)
...
ggsave("curves_global_mae.png", curves_global_mae, width = 15, height = 10, dpi = 600)
...
ggsave("curves_global_resid_range.png", curves_global_range, width = 15, height = 10, dpi = 600)
...
bar_threshold <- ggplot(threshold_corgw, aes(y = sp, x = continent, fill = index)) +
  geom_bar(position="dodge", stat="identity", col = "black") +
  facet_wrap(~index, scales = "free") +
  theme_bw() +
  scale_x_discrete(guide = guide_axis(angle = 45)) +
  scale_fill_brewer(palette = "Dark2")
ggsave("bar_threshold_095.png", bar_threshold, width = 10, height = 5, dpi = 600)


allv <- curves_global + bar_threshold
ggsave("all_threshold_095.png", allv, width = 10, height = 5, dpi = 600)


cor_comparison <- ggplot(samples_global, aes(y = mean_corgw, x = mean_pseudor, col = index)) +
  geom_point() +
  facet_wrap(~index, scales = "free") +
  theme_bw() +
  scale_color_brewer(palette = "Dark2")

ggsave("cor_comparison.png", cor_comparison, width = 15, height = 10, dpi = 600)

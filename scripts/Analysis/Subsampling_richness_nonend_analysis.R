...
list_data <- list.files("C:/Users/s2117440/OneDrive - Royal Botanic Garden Edinburgh/A_PhD/Chapters/Chapter1_UNEP_KEW/Projects/SubsamplingPlantBiodiversity/data/end_nonend/nonend", full.names = TRUE)
#Read them in a list
...
threshold_cont <- samples_cumulative_rel %>%
  mutate(threshold = case_when(mean > 0.7 & sp > 100 ~ 'above', 
                               mean < 0.7 & sp < 100 ~ 'below')) %>% 
...
overall_nonend_cor <-samples_cumulative_rel %>% 
  group_by(Continent) %>% 
  arrange(desc(sp)) %>% 
  slice_max(mean) %>% 
  slice_min(sp) %>% 
...
nonend_table
save_reactable(nonend_table, "nonend__summary_table_.png")#, vwdith = 2400)
...
ggsave(file = paste0("curves_nonendemic_cropdiv", 
                     "_increasingsteps.jpeg"),curves,   
       width = 22, height = 10, dpi = 600)

# richness maps 
index_nonendemic <- dist_native %>% 
  group_by(plant_name_id) %>% 
  summarise(n = n()) %>% 
  filter(n > 1) # 56%

richness_patterns_con_nonen <- dist_native %>%
  filter(plant_name_id %in% index_nonendemic$plant_name_id) %>% 
  group_by(area_code_l3) %>% 
  summarise(rich_nonen = n_distinct(plant_name_id)) %>% 
...
tdwg_3_mod <- tdwg_3 %>% 
  left_join(richness_patterns_con_nonen, by = c("LEVEL3_COD" = "area_code_l3")) 

pattern <- ggplot(data = tdwg_3_mod) +
  geom_sf(aes(fill = rich_nonen)) +
...
ggsave(file = paste0("maps_curves_nonendemic", 
                     "_cropdiv.jpeg"),g,   
       width = 22, height = 10, dpi = 600)

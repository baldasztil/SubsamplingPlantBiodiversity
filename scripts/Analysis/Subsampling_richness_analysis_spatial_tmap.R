...
tdwg_3_mod <- tdwg_3 %>% 
  ...
  filter(!LEVEL3_NAM == "Magadan" &
           !LEVEL3_NAM == "Fiji") # remove due to invalid geometry for bubble map


wrong <- tdwg_3 %>%   filter(LEVEL3_NAM == "Magadan" |
                      LEVEL3_NAM == "Fiji") 
wrong$geometry
...
#tdwg_wgs <- tdwg_3_mod[!st_is_valid(tdwg_3_mod)== TRUE,]

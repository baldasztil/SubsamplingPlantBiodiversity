...
table_data <- table(dist_native_calc$area_code_l3, dist_native_calc$plant_name_id)

traits <- growth %>%
  filter(plant_name_id %in% dist_native_calc$plant_name_id) %>% 
...
table_data <- as.data.frame.matrix(table(dist_native_calc$growth_form, dist_native_calc$area_code_l3))
x <- table_data[, 1:2]

x <- metacommunity(table_data)
x <- norm_sub_alpha(x, 0:2)


library(hillR)
table_data <- as.data.frame.matrix(table( dist_native_calc$area_code_l3, dist_native_calc$growth_form))
table_data <- as.data.frame.matrix(table( dist_native_calc$area_code_l3, dist_native_calc$family))

hill_taxa(table_data, q = 5)

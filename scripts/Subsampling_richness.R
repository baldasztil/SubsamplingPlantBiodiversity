...
ggarrange(rich_species_3000,rich_species_full, ncol = 2, nrow =1)
ggsave(paste0("speciesboxplot", "n",max(rich_rel_cumulative_names$sp),"_steps",diff(rich_rel_cumulative_names$sp)[1],".jpeg"),  width = 25, height = 8, dpi = 600)
...
ggsave(paste0("curves5000", "n",max(rich_rel_cumulative_names$sp),"_steps",diff(rich_rel_cumulative_names$sp)[1],".jpeg"),  width = 12, height = 8, dpi = 600)
...
for (i in 1:9) {
  ...
  aa <- rich_rel %>% 
    split(.$LEVEL1_COD) %>% 
    map_dbl(~cor.test(.$richness_sample, .$richness, method="spearman", data = ., exact =F )[[4]])
  aa <- as.data.frame(aa)   %>% 
    mutate(id = rownames(.))

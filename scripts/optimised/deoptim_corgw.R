...
subsampling.plants.solo <- function(spec_n) {
  ...
  if (cor.gwr > max(maxima)) {
    ...
    write.table(c(cor.gwr,ids), paste0("/exports/eddie/scratch/s2117440/data/",
...
solutions_listed <- mclapply(seq(2, nrow(plantlist_names), 34912), mc.cores = 30, deoptim_lapply)

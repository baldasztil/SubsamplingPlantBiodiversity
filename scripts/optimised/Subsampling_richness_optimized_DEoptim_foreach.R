...
mbm <- microbenchmark(
  "parallel_foreach" = { foreach(i = 1380:1390, .packages = c("tidyverse", "DEoptim"), .combine = "rbind") %dopar% {
...
de_result <- DEoptim(objective, lower = 855, upper = 855,
                     DEoptim.control(VTR = -Inf, strategy = 2,
                                     bs = FALSE, itermax = 100, CR = 0.5, F = 0.8, 
                                     storepopfreq = 1, trace = TRUE))

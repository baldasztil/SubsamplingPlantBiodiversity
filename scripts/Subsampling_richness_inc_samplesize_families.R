...
for (i in (1:length(plant_families))) {
  plantlist_names <- plantlist %>% 
    filter(family %in% plant_families[i])
  for (x in 1:100) {
      repeat{
        ...
        if (nrow(cumulative_namelist) < 10 & nrow(plantlist_names_left) > 0) {
          species_sample <- sample_n(plantlist_names_left, 1)
          }
          if (nrow(cumulative_namelist) >= 10 & nrow(plantlist_names_left) >= 10) {
            species_sample <- sample_n(plantlist_names_left, 10)
           }
           if (nrow(cumulative_namelist) >= 100 & nrow(plantlist_names_left) >= 100) {
                species_sample <- sample_n(plantlist_names_left, 100)
              }
                if (nrow(cumulative_namelist) > 100 & nrow(plantlist_names_left) < 100 
                    & nrow(plantlist_names_left) > 0) {
        ...
        rich_rel$family <- plant_families[i]
        ...
        rich_rel_bru$family <- plant_families[i]
        # per continent 
        cor.sp <- rich_rel %>% 
          split(.$LEVEL1_COD) %>% 
          ...
          rich_rel_con$family <- plant_families[i]
        ...
        if (nrow(cumulative_namelist) %in% seq(0,nrow(plantlist_names),100)){
        ...
        print(paste0("This is family ", plant_families[i]))
        end.time <- Sys.time()
        time.taken <- end.time - start.time
        print(time.taken)
  }
  data_out <- as.data.frame(do.call(rbind, samplelist))
  write.table(data_out, paste0("output/families/", plant_families[i],".txt"))
}

...
model_preds <-  function(model) {
  temp <- as.data.table(model$obdata)[, c("LEVEL1_NAM", "LEVEL2_COD", "richness", "richness_sample")] %>%
...
cor.gw <- function(rich_rel_shp) {
  ...
  if (class(stats)[1]=="try-error") {
    ...
    stats_total <-  cbind(stats, rich_rel_shp) %>% 
      dplyr::select(cor.sp_gwr = Spearman_rho_richness.richness_sample, LEVEL2_COD, 
...
subsampling.plants <- function(spec_n) {
  ...
  richness_rel <- rich_rel_rich_tax %>% 
    left_join(rich_rel_pd, by = "LEVEL2_COD") %>% 
    left_join(rich_rel_fun, by = "LEVEL2_COD")
  ...
  rich_rel <-  rich_rel_long %>% 
    right_join(rich_overall_bru, by = c("LEVEL2_COD", "dim")) %>%
    left_join(midpoints_red, by = c("LEVEL2_COD")) %>% 
    replace(is.na(.), 0) %>% 
...
tdwg_3 <- st_read(dsn ="data/wgsrpd-master/level3") %>% 
  mutate(LEVEL2_COD = as.character(LEVEL2_COD)) %>% 
  filter(!duplicated(LEVEL2_COD))
  

continent_names <- tdwg_3 %>% 
  dplyr::select(LEVEL1_NAM, LEVEL2_COD, LEVEL3_COD) %>% 
  filter(!duplicated(LEVEL2_COD)) %>% 
  st_drop_geometry()
...
table_data <- table(dist_native_calc$region_code_l2, dist_native_calc$growth_form)


shannon_div <- as.data.frame(diversity(table_data, index = "shannon")) 
shannon_div$C <- rownames(shannon_div)
names(shannon_div) <- c("richness_fun", "LEVEL2_COD")
...
plantlist_dist_phylo_growth <- dist_native %>% 
  ...
  dplyr::select(plant_name_id, taxon_name, LEVEL2_COD = region_code_l2, growth_form, family) %>% 
  mutate(label = gsub(" ", "_", taxon_name))

dist.mat <- long2sparse(plantlist_dist_phylo_growth, grids = "LEVEL2_COD", species = "label")
...
richness_patterns_rich <- dist_native_calc %>% 
  ...
  summarise(richness = length(unique(plant_name_id))) %>% 
  mutate(LEVEL2_COD = as.character(region_code_l2))
...
richness_patterns <-richness_patterns_rich %>% 
  left_join(richness_patterns_tax, by = "LEVEL2_COD") %>% 
  left_join(richness_patterns_phy, by = "LEVEL2_COD") %>% 
  left_join(richness_patterns_fun, by = "LEVEL2_COD") %>% 
  left_join(continent_names, by = "LEVEL2_COD") 
...
rich_overall_bru_mid <- rich_patterns_long %>% 
  left_join(midpoints_red, by = "LEVEL2_COD")  %>% 
  mutate(richness2 = richness) %>% 
  st_as_sf()

rich_overall_bru_shp <- rich_patterns_long %>% 
  left_join(tdwg_3, by = "LEVEL2_COD") %>% 
  st_as_sf()
...
result_list <- lapply(seq(2,nrow(plantlist_names),1000), 
                      subsampling.plants)



xx <- rbindlist(result_list)


fwrite(xx, "results_samples_region_by_1000.txt")
